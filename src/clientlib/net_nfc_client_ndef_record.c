...
NET_NFC_EXPORT_API net_nfc_error_e net_nfc_create_uri_string_from_uri_record(ndef_record_h record, char ** uri)
{
	...
	if(net_nfc_get_record_type(record, &rec_type) == NET_NFC_OK)
	{
		if(((data_s *)rec_type)->length > 0)
		{
			if(strncmp((char *)net_nfc_get_data_buffer(rec_type), URI_RECORD_TYPE , 1) != 0)
			{
				DEBUG_CLIENT_MSG("record type is not matched");
				return NET_NFC_NDEF_RECORD_IS_NOT_EXPECTED_TYPE;
			}
	...
	DEBUG_CLIENT_MSG("this is URL type");
	...
	if(net_nfc_get_record_payload(record, &payload) == NET_NFC_OK)
	{
		...
		if(buffer_temp[0] == 0x00)
		{
			*uri = (char *)calloc(1, buffer_length); /* buffer length include a schema byte. so it does not need to allocate one more byte for string. */

			if(*uri != NULL)
			{
				memcpy(*uri, &buffer_temp[1], buffer_length - 1);
				DEBUG_CLIENT_MSG("the uri to open is = [%s]", *uri);
			}
			else
			{
				return NET_NFC_ALLOC_FAIL;
		...
		{
			if(buffer_temp[0] >= NET_NFC_SCHEMA_MAX)
			{
				DEBUG_CLIENT_MSG("unknow schema. abandon \n");
			}
			else
			{
				DEBUG_CLIENT_MSG("uri schema is %s", net_nfc_util_get_schema_string(buffer_temp[0]));

				int uri_schema_length = strlen(net_nfc_util_get_schema_string(buffer_temp[0]));
				int uri_buffer_length = uri_schema_length + buffer_length;

				*uri = (char *)calloc(1, uri_buffer_length);

				if(*uri != NULL)
				{
					memcpy(*uri, net_nfc_util_get_schema_string(buffer_temp[0]), uri_schema_length);
					memcpy(*uri + uri_schema_length, &buffer_temp[1], buffer_length - 1);

					DEBUG_CLIENT_MSG("the url to launch = [%s]", *uri);
				}
				else
				{
					return NET_NFC_ALLOC_FAIL;
