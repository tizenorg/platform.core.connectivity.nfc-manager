...
NET_NFC_EXPORT_API net_nfc_error_e net_nfc_create_data (data_h* data, const uint8_t* bytes, const uint32_t length)
{
	...
	_net_nfc_client_util_alloc_mem (*data_private,sizeof (data_s));

	if (*data_private == NULL) return NET_NFC_ALLOC_FAIL;

	if (length <= 0){
		return NET_NFC_OK;
	}

	_net_nfc_client_util_alloc_mem ((*data_private)->buffer, length);
	if ((*data_private)->buffer == NULL )return NET_NFC_ALLOC_FAIL;
...
NET_NFC_EXPORT_API net_nfc_error_e net_nfc_set_data (const data_h data, const uint8_t* bytes, const uint32_t length)
{
	...
	if (data_private->buffer != NULL) {
		_net_nfc_client_util_free_mem (data_private->buffer);
	}
...
NET_NFC_EXPORT_API net_nfc_error_e net_nfc_free_data (data_h data)
{
	...
	_net_nfc_client_util_free_mem (data_private);
