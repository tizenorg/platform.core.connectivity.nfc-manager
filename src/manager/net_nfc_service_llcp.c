...
bool net_nfc_service_llcp_process(net_nfc_target_handle_s* handle, int devType, net_nfc_error_e* result)
{
	*result = NET_NFC_OK;
		...
		if(net_nfc_controller_llcp_activate_llcp(handle, result) == true)
		{
			...
			if(net_nfc_service_get_configuration_value("service", "exchange_service", value) == NET_NFC_OK)
			{
				if(strcmp(value, "true") == 0)
				{
					...
					_net_nfc_manager_util_alloc_mem (exchange_state_server,sizeof (net_nfc_llcp_state_t));
			...
			if(net_nfc_service_get_configuration_value("service", "connection_handover", value) == NET_NFC_OK){
				if(strcmp(value, "true") == 0){
					...
					_net_nfc_manager_util_alloc_mem (conn_handover_selector,sizeof (net_nfc_llcp_state_t));
			...
			_net_nfc_manager_util_alloc_mem (exchange_state_server_NPP,sizeof (net_nfc_llcp_state_t));
			...
			_net_nfc_manager_util_alloc_mem (exchange_state_server_SNEP,sizeof (net_nfc_llcp_state_t));
	...
	if (_net_nfc_check_client_handle ())
	{
		...
		_net_nfc_send_response_msg (NET_NFC_MESSAGE_P2P_DISCOVERED, &req_msg, sizeof (net_nfc_response_p2p_discovered_t), NULL);
	}
...
bool net_nfc_service_llcp_disconnect_target(net_nfc_request_msg_t *msg)
{
	...
	{
		DEBUG_SERVER_MSG("the target ID = [0x%x] was not connected before. current device may be a TARGET");
	}
	...
	if (_net_nfc_check_client_handle())
	{
		...
		_net_nfc_send_response_msg(NET_NFC_MESSAGE_P2P_DETACHED, &message, sizeof(net_nfc_response_llcp_detached_t), NULL);
	}
...
bool net_nfc_service_llcp_process_socket_error(net_nfc_request_msg_t *msg)
{
	...
	if (_net_nfc_check_client_handle())
	{
		...
		return _net_nfc_send_response_msg(NET_NFC_MESSAGE_LLCP_ERROR, error, sizeof(net_nfc_response_llcp_socket_error_t), NULL);
	}
...
bool net_nfc_service_llcp_process_connect_sap_socket(net_nfc_request_msg_t *msg)
{
	...
	if (_net_nfc_check_client_handle())
	{
		...
		return _net_nfc_send_response_msg(NET_NFC_MESSAGE_LLCP_CONNECT_SAP, detail, sizeof(net_nfc_response_connect_sap_socket_t), NULL);
	}
	res = _net_nfc_service_llcp_state_process(msg);
...
bool net_nfc_service_llcp_process_send_to_socket(net_nfc_request_msg_t* msg)
{
	...
	if (_net_nfc_check_client_handle())
	{
		...
		detail->result = llcp_msg->result;
		return _net_nfc_send_response_msg(NET_NFC_MESSAGE_LLCP_SEND_TO, detail, sizeof(net_nfc_response_send_socket_t), NULL);
	}
	res = _net_nfc_service_llcp_state_process(msg);
...
bool net_nfc_service_llcp_process_receive_from_socket(net_nfc_request_msg_t *msg)
{
	...
	if (_net_nfc_check_client_handle())
	{
		...
		detail->result = llcp_msg->result;
		return _net_nfc_send_response_msg(NET_NFC_MESSAGE_LLCP_RECEIVE_FROM, detail, sizeof(net_nfc_response_receive_socket_t),
...
bool net_nfc_service_llcp_process_disconnect_socket(net_nfc_request_msg_t *msg)
{
	...
	if (_net_nfc_check_client_handle())
	{
		...
		detail->result = llcp_msg->result;
		return _net_nfc_send_response_msg(NET_NFC_MESSAGE_LLCP_DISCONNECT, detail, sizeof(net_nfc_response_disconnect_socket_t), NULL);
	}
...
net_nfc_error_e net_nfc_service_send_exchanger_msg(net_nfc_request_p2p_send_t *msg)
{
	...
	_net_nfc_manager_util_alloc_mem(ex_data, sizeof(data_s));
	if (ex_data == NULL)
	{
	...
	_net_nfc_manager_util_alloc_mem(ex_data->buffer, ex_data->length);
	if (ex_data->buffer == NULL)
	{
		DEBUG_SERVER_MSG(" alocation is failed");
		_net_nfc_manager_util_free_mem(ex_data);
		return NET_NFC_ALLOC_FAIL;
	}
	memcpy(ex_data->buffer, &msg->data.buffer, msg->data.length);

	_net_nfc_manager_util_alloc_mem(exchange_state_client, sizeof(net_nfc_llcp_state_t));
	if (exchange_state_client == NULL)
	{
		DEBUG_SERVER_MSG(" alocation is failed");
		_net_nfc_manager_util_free_mem(ex_data);
		_net_nfc_manager_util_free_mem(ex_data->buffer);
		return NET_NFC_ALLOC_FAIL;
	}
	...
	exchange_state_client->user_data = ex_data;
	...
	*result = NET_NFC_OK;
	...
	switch (state->step)
	{
		...
		case NET_NFC_LLCP_STEP_02:
		{
			...
			_net_nfc_manager_util_alloc_mem (new_client,sizeof (net_nfc_llcp_state_t));
		...
		case NET_NFC_LLCP_STEP_03:
		{
			...
			if(_net_nfc_service_llcp_snep_check_req_msg(&snep_server_data, &resp_code) != NET_NFC_OK)
			{
				...
				if(net_nfc_controller_llcp_send(state->handle, state->socket, resp_msg, result, state) == false)
				{
					...
					_net_nfc_manager_util_free_mem(resp_msg->buffer);
					_net_nfc_manager_util_free_mem(resp_msg);
				}
				...
				if(_net_nfc_service_llcp_snep_get_information_length(&snep_server_data, &information_length) == NET_NFC_OK)
				{
					...
					if(information_length + 6 > SNEP_MAX_BUFFER){
						...
						data_s* fragment = (data_s *)calloc(1, sizeof(data_s));
						...
						fragment->buffer = (uint8_t*)calloc(information_length + 6, sizeof(uint8_t));

						if(fragment->buffer == NULL){
				 			state->step = NET_NFC_STATE_ERROR;
							_net_nfc_manager_util_free_mem (fragment);
							break;
						}
						 ...
						 if((_net_nfc_service_llcp_snep_get_information_length(&snep_server_data, &(temp.length))) == NET_NFC_OK)
						 {
							...
							if(net_nfc_server_get_current_client_context(&client_context) == true)
							{
								if(net_nfc_server_check_client_is_running(&client_context) == true)
								{
									...
									_net_nfc_manager_util_alloc_mem (data.buffer, temp.length);
									...
									_net_nfc_send_response_msg (NET_NFC_MESSAGE_P2P_RECEIVE, (void*)&resp, sizeof (net_nfc_response_p2p_receive_t),
		...
		case NET_NFC_LLCP_STEP_06:
		{
			...
			if(((data_s*)state->user_data)->length > (snep_server_data.length + state->fragment_offset))
			{

				/* receive more */
				/* copy fragment to buffer. */
				data_s* fragment = state->user_data;
				if(fragment != NULL)
				{
			...
			else if(((data_s*)state->user_data)->length == (snep_server_data.length + state->fragment_offset)){
				...
				/* version, command, information_length are head. */
				temp.buffer = ((data_s*)(state->user_data))->buffer + sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint32_t);
				 if((_net_nfc_service_llcp_snep_get_information_length(((data_s*)(state->user_data)), &(temp.length))) == NET_NFC_OK)
				 {
				 ...
				 _net_nfc_manager_util_free_mem(((data_s*)(state->user_data))->buffer);
				 _net_nfc_manager_util_free_mem(state->user_data);
			...
			else
			{
				...
				if(_net_nfc_service_llcp_npp_get_information_length(&snep_server_data, &information_length) == NET_NFC_OK){
					...
					if(information_length + 10 > SNEP_MAX_BUFFER)
					{
						...
						fragment->buffer = (uint8_t*)calloc(information_length + 10, sizeof(uint8_t));
	...
	*result = NET_NFC_OK;
	...
	switch (state->step)
	{
		...
		case NET_NFC_LLCP_STEP_02:
		{
			...
			{
				req_msg = _net_nfc_service_llcp_snep_create_msg(SNEP_REQ_PUT, (data_s *)state->user_data);
			}
			else
			{
				req_msg = _net_nfc_service_llcp_npp_create_msg((data_s *)state->user_data);
			}
			...
			if(req_msg->length <= state->max_capability)
			{
				...
				if((fragment.buffer = (uint8_t *)calloc(state->max_capability, sizeof(uint8_t))) == NULL)
				{
					_net_nfc_manager_util_free_mem (req_msg->buffer);
					_net_nfc_manager_util_free_mem (req_msg);
					state->user_data = NULL;
				...
				if((extra_msg = (data_s*)calloc(1, sizeof(data_s))) == NULL)
				{
					_net_nfc_manager_util_free_mem (req_msg->buffer);
					_net_nfc_manager_util_free_mem (req_msg);
					_net_nfc_manager_util_free_mem(fragment.buffer);
					state->user_data = NULL;
		 			state->step = NET_NFC_STATE_ERROR;
					break;
				}

				if((extra_msg->buffer = calloc(req_msg->length - state->max_capability, sizeof(uint8_t))) == NULL)
				{
					_net_nfc_manager_util_free_mem (req_msg->buffer);
					_net_nfc_manager_util_free_mem (req_msg);
					_net_nfc_manager_util_free_mem(extra_msg);
					_net_nfc_manager_util_free_mem(fragment.buffer);
					state->user_data = NULL;
		 			state->step = NET_NFC_STATE_ERROR;
					break;
				...
				if(_net_nfc_service_llcp_snep_get_code(&snep_client_data, &code) == NET_NFC_OK)
				{
					...
					else if(code == SNEP_RESP_CONT)
					{
						...
						if(msg->length > state->max_capability)
						{
							...
							if((fragment.buffer = (uint8_t *)calloc(state->max_capability, sizeof(uint8_t))) == NULL)
							{
								_net_nfc_manager_util_free_mem(msg->buffer);
								_net_nfc_manager_util_free_mem(msg);
								state->user_data = NULL;
					 			state->step = NET_NFC_STATE_ERROR;
								break;
							}
							...
							if((extra_msg = calloc(1, sizeof(data_s))) == NULL)
							{
								_net_nfc_manager_util_free_mem(fragment.buffer);
								_net_nfc_manager_util_free_mem(msg->buffer);
								_net_nfc_manager_util_free_mem(msg);
							...
							if((extra_msg->buffer = calloc(msg->length - state->max_capability, sizeof(uint8_t))) == NULL)
							{
								_net_nfc_manager_util_free_mem(extra_msg);
								_net_nfc_manager_util_free_mem(fragment.buffer);
								_net_nfc_manager_util_free_mem(msg->buffer);
		...
		case NET_NFC_LLCP_STEP_05:
		{
			...
			if(msg->length > state->max_capability)
			{
				...
				fragment.buffer = (uint8_t *)calloc(state->max_capability, sizeof(uint8_t));
				fragment.length = state->max_capability;
				memcpy(fragment.buffer, msg->buffer, state->max_capability);

				extra_msg = calloc(1, sizeof(data_s));

				extra_msg->buffer = calloc(msg->length - state->max_capability, sizeof(uint8_t));
				extra_msg->length = msg->length - state->max_capability;
				memcpy(extra_msg->buffer, msg->buffer + state->max_capability, msg->length - state->max_capability);
...
static data_s* _net_nfc_service_llcp_snep_create_msg(snep_command_field_e resp_field, data_s* information)
{
	...
	if(information == NULL){
		...
		if((snep_msg->buffer = calloc(snep_msg->length, sizeof(uint8_t))) == NULL){
			...
			_net_nfc_manager_util_free_mem(snep_msg);
			return NULL;
		}
...
static data_s* _net_nfc_service_llcp_npp_create_msg( data_s* information)
{
	...
	if(information == NULL)
	{
		...
		if((npp_msg = (data_s*)calloc(1, sizeof(data_s))) == NULL)
		{
			return NULL;
		}
		...
		if((npp_msg->buffer = calloc(npp_msg->length, sizeof(uint8_t))) == NULL){

			_net_nfc_manager_util_free_mem(npp_msg);
			return NULL;
		}
	...
	else
	{
		...
		if((npp_msg->buffer = (uint8_t *)calloc(npp_msg->length, sizeof(uint8_t))) == NULL)
		{
			_net_nfc_manager_util_free_mem(npp_msg);
			return NULL;
		}
...
static net_nfc_error_e _net_nfc_service_llcp_npp_get_information_length(data_s* npp_msg, uint32_t* length)
{
	...
	DEBUG_SERVER_MSG("check the npp ndef length address= [%x]" , temp);
